...
// Author...: G. Wassink
// Design...:
// Date.....: 12/06/2024 Last revised: 25/09/2024
...
{
   internal static bool ActivePowerACLimitApi(this ModbusApiClient target, short percent = 100)
   {
      if (percent >= 0 && percent <= 100)
      {
         target.WriteSingleRegister(2, 40016, percent);                                                                                                              // Normalized Active Power Limition by PV system ctrl -> Range 0% to 100% -> default 100% (write only)
   ...
   internal static void BatteryControlApi(this ModbusApiClient target, BatteryControlEnum control = BatteryControlEnum.Inverter)
   {
      target.WriteMultipleRegisters<uint>(3, 40151, control == BatteryControlEnum.Remote ? [802] : [803]);                                                           // 802 -> set control to remote : 803 -> set control to inverter
   }
