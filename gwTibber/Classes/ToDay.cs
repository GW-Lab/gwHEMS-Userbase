...
public class ToDay
{
   ...
   public ToDay(EPEX.PriceInfo data, double rise, double set)
   {
      ...
      if (this.PricesInfo.Count is 96 or 92 or 100)                           //	Summer winter time switch 24 hours?  
      {
         ...
         BatteryTimeShiftChargingToTimeSlot = GetTimeSlotForConsecutiveEPEXPrices(TimeSlot(new TimeOnly(TS(this.SunRise).Hours, TS(this.SunRise).Minutes)),consecutiveEPEXSpotPrices: 4, EPEXPrice.Minimum);
         MaxPriceTimeSlotAfterBatteryTimeShiftChargingTimeSlot = GetTimeSlotForConsecutiveEPEXPrices(BatteryTimeShiftChargingToTimeSlot + 1, consecutiveEPEXSpotPrices: 4, price: EPEXPrice.Maximum);

         Valide = true;
      }
   }

   private int GetTimeSlotForConsecutiveEPEXPrices(int startTimeSlot, int consecutiveEPEXSpotPrices, EPEXPrice price = EPEXPrice.Maximum)
   {
      ...
      for (int ndx = startTimeSlot + 1; ndx <= PricesInfo.Count - consecutiveEPEXSpotPrices; ndx++)
      {
