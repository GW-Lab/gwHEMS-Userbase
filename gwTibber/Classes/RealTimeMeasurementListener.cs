// Program..: HomeRealTimeMeasurementObservable.cs
// Author...: G. Wassink
// Design...:
...
internal class HomeRealTimeMeasurementObservable(RealTimeMeasurementListener listener, Guid homeId, int subscriptionId) : IObservable<RealTimeMeasurement>
{
   private readonly RealTimeMeasurementListener _listener = listener;

   public Guid HomeId { get; } = homeId;
...
internal class RealTimeMeasurementListener : IDisposable
{
   ...
   private readonly Tibber _tibber;

   private readonly Dictionary<Guid, HomeStreamObserverCollection> _homeObservables = [];
   ...
   public RealTimeMeasurementListener(Tibber tibberApiClient, Uri websocketSubscriptionUrl, string accessToken)
   {
      _tibber = tibberApiClient;
   ...
   public async Task<IObservable<RealTimeMeasurement>> SubscribeHome(Guid homeId, CancellationToken cancellationToken)
   {
      ...
      {
         shouldInitialize = _homeObservables.Count == 0;
   ...
   private async Task Initialize(Uri websocketSubscriptionUrl, CancellationToken cancellationToken)
   {
...
#if NETFRAMEWORK
            connectionInitPayload.UserAgent = Tibber.UserAgent.ToString();
#else
      _wssClient.Options.SetRequestHeader("User-Agent", Tibber.UserAgent.ToString());
#endif
      ...
      var json = JsonConvert.SerializeObject(connectionInitMessage, Tibber.JsonSerializerSettings);
   ...
   private async Task TryReconnect()
   {
      ...
      while (!_cancellationTokenSource.IsCancellationRequested)
      {
         try
         {
            ...
            var homes = await _tibber.ValidateRealtimeDevice();
   ...
   {
      var jitter = Random.Next(5, 60);                               // Jitter of 5 to 60 seconds
      var delay = Math.Pow(failures, 2);                             // Exponential backoff
      const double oneDayInSeconds = (double)60 * 60 * 24;           // Max one day 60 * 60 * 24
   ...
   {
      public readonly List<IObserver<RealTimeMeasurement>> Observers = [];
