...
namespace gwTibber.Classes;
    ...
    public class ApiHttpException : ApiException
    {
        ...
        private ApiHttpException(Uri uri, HttpMethod httpMethod, HttpStatusCode statusCode, string reasonPhrase, HttpRequestHeaders requestHeaders, HttpResponseHeaders responseHeaders, TimeSpan requestDuration, string message)
             : base(message)
        {
            Uri = uri;
        ...
        internal ApiHttpException(Uri uri, HttpMethod httpMethod, TimeSpan requestDuration, string message = null, Exception exception = null)
        ...
        internal static async Task<ApiHttpException> Create(Uri uri, HttpMethod httpMethod, HttpResponseMessage response, TimeSpan duration, string message = null)
        {
            var exception = new ApiHttpException(uri, httpMethod, response.StatusCode, response.ReasonPhrase, response.RequestMessage.Headers, response.Headers, duration, message ?? $"HTTP '{httpMethod} {uri}' call failed with status '{response.ReasonPhrase}' ({(int)response.StatusCode})");
        ...
        //        {
        //            builder.Append(RequestContent.AsSpan(0, MaximumBodyLength));
        //            builder.AppendLine("\u2026");
        //        }
        //        else
        ...
        //        {
        //            builder.Append(ResponseContent.AsSpan(0, MaximumBodyLength));
        //            builder.AppendLine("\u2026");
        //        }
        //        else
