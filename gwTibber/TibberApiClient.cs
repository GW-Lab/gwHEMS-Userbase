...
// Author...: G. Wassink
// Design...:
// Date.....: 12/06/2024 Last revised: 21/09/2024
// Notice...: Copyright 1999, All Rights Reserved
// Notes....: C#13 .Net 9.0.10
// Files....: None
// Programs.:
// Reserved.: Type Class (TibberApiClient)
...
public class TibberApiClient(string accessToken, double lat = 0.0, double lng = 0.0)
{
	private const double SunriseSunsetAltitude = -35d / 60d;
	private const double INV360 = 1.0d / 360.0d;

	private readonly string accessToken = accessToken;   // "BIZ04Kr9-nWq33gnMvUus_hREWH5X4NDbCk46NJI6L8";
	private readonly string toDayAndToMorrowQuery = "{viewer{homes{currentSubscription{priceInfo{current{total energy tax startsAt}today{total energy tax startsAt}tomorrow{total energy tax startsAt}}}}}}";
	private readonly ProductInfoHeaderValue userAgent = new("Gijs-home-automation-system", "1.0");                                // Set a user agent for tracking different client implementations

	public ToDay ToDay;
	public ToMorrow ToMorrow;

	public bool GetEPEX(int year, int month, int day)
	{
		try
		{
			using var client = new Tibber(accessToken, userAgent);
			var data = client.Query(toDayAndToMorrowQuery).Result;

			SunriseSunset(year, month, day, lng, lat, SunriseSunsetAltitude, true, out double riseToDay, out double setToDay);
			ToDay = new ToDay(data, riseToDay, setToDay);                                                                            // on availability -> Populate 

			SunriseSunset(year, month, day, lng, lat, SunriseSunsetAltitude, true, out double riseToMorrow, out double setToMorrow);
			ToMorrow = new ToMorrow(data, riseToMorrow, setToMorrow);                                                                // on availability -> Populate 
	...
	private double DayLenght(int year, int month, int day, double lon, double lat, double altit, bool upper_limb)
	{
		...
		switch (cost)                                                  // Sun always below altit 
		{
			case >= 1.0: return 0.0;                                    // Diurnal arc
			case <= -1.0: return 24.0;                                  // Diurnal arc
			default: return 2.0 / 15.0 * Acosd(cost);                   // Diurnal arc
		}
	}
