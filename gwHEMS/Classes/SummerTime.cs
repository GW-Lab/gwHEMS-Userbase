...
internal class SummerTime(TibberApiClient tb, ChargerApiClient wb, ModbusApiClient hm, ModbusApiClient pv, EnvilineApiClient hp)
{
   /* Process variables */
   ...
   private const int ZeroExport = 0;                                                                      // HM Set Zero Export  Export=0%
   private const int NormalExport = 100;                                                                 // HM Set Zero Export  Export=100%
   ...
   public void Process(DateTime currDateTime, OptimizationTarget optimization)
   {
      ...
      // wb.SetChargeMode(ChargingModeEnum.PVSurPlus);
      ...
      {
         PVPlantZeroExportCharging(optimization);
      }
      else                                                                                               // Positive energy prices (EVCharger is not charging)  
      {
         if (status == ProcessStatus.BatteryChargeTimeShift)                                             // HEMS is in battery delay mode  
         {
            ...
            {
               hm.ActivePowerACLimit(NormalExport, pv);                                                  // Cancel zero export mode (Energy prices are positive)
               pv.Battery.PowerAC = (int)Battery.Power.Blocked;                                          // Battery block charging
            }
         }
         else if (expectedDayYield &&
                 ...
                 pv.Battery.PowerAC <= 0)                                                                // Battery power = 0 Watt (not discharging)
         {
   ...
   private void PVPlantZeroExportCharging(OptimizationTarget optimization)
   {
      hm.ActivePowerACLimit(ZeroExport, pv);                                                          // Set zero export mode (BatteryChargeFromPV is also triggert)

      if (status == ProcessStatus.BatteryChargeTimeShift)                                             // HEMS is in battery delay mode     
      {
         if (optimization == OptimizationTarget.Economical)
            pv.Battery.PowerACForced = (int)Battery.Power.ChargeMin;
         else
            pv.Battery.Control = BatteryControl.Inverter;
      }
      else
      {
         if (optimization == OptimizationTarget.Economical)
            pv.Battery.PowerACForced = (int)Battery.Power.ChargeMax;
         else
            pv.Battery.Control = BatteryControl.Inverter;
      }
   }
   ...
   private void BallBoxFastCharging(DateTime currDateTime, OptimizationTarget optimization)
   {
      ...
      if (status == ProcessStatus.BatteryChargeTimeShift)                                 // To Do Battary allways => SOC >= 50% 
      {
         ...
         {
            hm.ActivePowerACLimit(ZeroExport, pv);                                        // Set zero export mode (Energy prices are negative)

            if (optimization == OptimizationTarget.Economical)
               pv.Battery.PowerACForced = (int)Battery.Power.ChargeMin;                   // Set BatteryControll = Remote and charge battery with 1000 watt
            else
               pv.Battery.PowerAC = (int)Battery.Power.Blocked;
         ...
         else                                                                             // Positive prices get energy from the Battery? 
         {
            ...
            if (optimization == OptimizationTarget.Economical)
               pv.Battery.PowerAC = (int)Battery.Power.Blocked;                           // Set BatteryControll = Remote and charge battery with 1000 watt
            else
               pv.Battery.PowerAC = (int)Battery.Power.Blocked;
      ...
      else                                                                                // Battery charging not delayed
      {
         if (tb.ToDay.Prices(startsAt: currDateTime).Energy < MyBuyPrice)                 // Negative prices get energy (allways) from the GRID 
         {
            ...
            if (optimization == OptimizationTarget.Economical)
               pv.Battery.PowerACForced = (int)Battery.Power.ChargeMax;                   // Set BatteryControll = Remote and charge battery with max watt
            else
               pv.Battery.PowerACForced = (int)Battery.Power.ChargeMin;                   // Set BatteryControll = Remote and charge battery with min watt
   ...
   private void WallBoxPVSurPlusCharging(DateTime currDateTime, OptimizationTarget optimization)
   {
      ...
      if (status == ProcessStatus.BatteryChargeTimeShift)
      {
         if (energyPrice < MyBuyPrice)                                                     // Negative prices get energy (allways) from the GRID 
         {
            ...
            if (optimization == OptimizationTarget.Economical)
               pv.Battery.PowerACForced = (int)Battery.Power.ChargeMax;                   // Set BatteryControl = Remote and charge battery with 2000 watt
            else
      ...
      else                                                                                // Battery charging not delayed
      {
         if (energyPrice < MyBuyPrice)                                                    // Negative prices get energy (allways) from the GRID 
         {
            ...
            if (optimization == OptimizationTarget.Economical)
               pv.Battery.PowerACForced = (int)Battery.Power.ChargeMax;                   // Set BatteryControl = Remote and charge battery with 1000 watt
            else
               pv.Battery.Control = BatteryControl.Inverter;                              // Set BatteryControl = Inverter and charge the battery with PV   
   ...
   private void IdleState(DateTime currDateTime)
   {
      ...
      {
         pv.Battery.PowerACForced = (int)Battery.Power.ChargeMin;                          // Set BatteryControll = Remote and Discharge battery with 5500 watt
      ...
      else if (status != ProcessStatus.Idle)                                              // HEMS idle mode
      {
         ...
         hm.ActivePowerACLimit(NormalExport, pv);                                         // Cancel Zero export mode (automatically triggert -> BatteryChargeFromPV)   
