...
internal class WinterTime(TibberApiClient tb, ChargerApiClient wb, ModbusApiClient hm, ModbusApiClient pv, EnvilineApiClient hp)
{
   ...
   private const int DefaultExport = 100;                                                             // HM Set Zero Export  Export=100%
   ...
   private void WallBoxPVSurPlusCharging(DateTime currDateTime, OptimizationTarget optimization)
   {
      ...
      else                                                                                      // Battery charging not delayed
      {
         ...
         {
            if (optimization == OptimizationTarget.Economical)
               pv.Battery.PowerACForced = (int)Battery.Power.ChargeMax;                         // Set BatteryControll = Remote and charge battery with 1000 watt
            else
               pv.Battery.Control = BatteryControl.Inverter;
         }
         else
   ...
   private void IdleState(DateTime currDateTime)
   {
      ...
      {
         pv.Battery.PowerACForced = (int)Battery.Power.ChargeMax;                               // BatteryChargePowerMax; // Set BatteryControll = Remote and Charge battery with 3000 watt
      ...
      {
         pv.Battery.PowerACForced = (int)Battery.Power.ChargeMin;                               //  WBBatteryDischargePowerMin; // Set BatteryControll = Remote and Discharge battery with 5500 watt
      ...
      {
         pv.Battery.Control = BatteryControl.Inverter;
         hm.ActivePowerACLimit(DefaultExport, pv);                                                        // Cancel Zero export mode (automatically triggert -> BatteryChargeFromPV)  
